...
import { scan, sweep, claim, view, setExtras, connectOauth, addRoute, removeRoute, confirmJourney, applyConfig, backfillRoutes } from "./app.js";
...
export function createServer(ctx: Ctx) {
  return createHttp(async (req, res) => {
    try {
      ...
      if (req.method === "POST" && url === "/api/oauth/connect") {
        // Opens the system browser for consent; resolves once the loopback captures the code.
        await connectOauth(ctx, await readJson(req));
        await scan(ctx);
        return send(res, 200, view(ctx));
      }
      if (req.method === "POST" && url === "/api/config") {
        applyConfig(ctx, await readJson(req)); // persists + rebuilds provider / demo flag
        await scan(ctx); // re-scan with the new settings, no-op merge if unchanged
